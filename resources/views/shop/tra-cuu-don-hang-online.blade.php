...
<script src="{{ asset('assets/js/plugins/date.js') }}"></script>
...
cta.addEventListener('click',function(){
    ...
    console.log(sdt);
    var url ='http://127.0.0.1:8000/bill/?sdt='+sdt;
    console.log(url);
    var str = ``;
    fetch(url)
    .then(response => response.json())
    .then(data => {
        ...
        if(data.code == 201 ){
            alertDone('Thông báo',data.mess);
        }else{
            data.mess.forEach(function (item) {
                str += `<tr>`;
                str += `<td>`+ item.name +`</td>`;
                str += `<td>`+ item.phone_number +`</td>`;
                str += `<td>`+ item.email +`</td>`;
                str += `<td>`+ item.money +`</td>`;
                str += `<td>`+ item.status.name +`</td>`;
                str += `<td>`+ item.products_count +`</td>`;
